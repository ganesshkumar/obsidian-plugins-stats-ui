...
export const CategoryIcon = ({ category, size = 48 }: { category: string, size: number }) => {
  return <Tooltip content={`${category} Plugin`}>
    <CategoryIconInternal category={category} size={size} />
  </Tooltip>
};

const CategoryIconInternal = ({ category, size = 48 }: { category: string, size: number }) => {
  switch (category) {
