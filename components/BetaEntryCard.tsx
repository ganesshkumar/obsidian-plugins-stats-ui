...
const customPluginCardTheme: CustomFlowbiteTheme['card'] = {
  root: {
    children: 'flex h-full flex-col justify-start gap-2 py-2 px-5 hover:bg-violet-100',
  },
};

const customThemeCardTheme: CustomFlowbiteTheme['card'] = {
  root: {
    children: 'flex h-full flex-col justify-start gap-2 py-2 px-5 hover:bg-emerald-100',
  },
};
...
export const BetaEntryCard = ({ entry, highlight }: IBetaEntryCardProps) => {
  ...
  const typeColor = entry.type === 'theme' ? 'bg-emerald-600 text-emerald-100' : 'bg-violet-600 text-violet-100';
  return (
    <Card theme={entry.type === 'theme' ? customThemeCardTheme : customPluginCardTheme}>
    {/* <div className="relative flex flex-col justify-between group shrink-0 my-1 px-5 py-2 border rounded-md shadow-lg hover:shadow-gray-200/50 shadow-slate-200/50 hover:bg-gray-50 bg-white text-gray-700"> */}
      <div>
        <div className="flex items-start justify-between gap-x-2">
          ...
          <span className={`px-2 py-0.5 uppercase rounded-full text-[10px] font-semibold ${typeColor}`}>{entry.type}</span>
        </div>
        <div className="text-xs">
          <span>{moment(entry.createdAt).fromNow()}</span>{' '}
          {entry.author && (
            <>
              by <span className="italic text-gray-600 group-hover:text-gray-700" dangerouslySetInnerHTML={{
                __html: highlight ? highlightMatches(entry.author, highlight) : entry.author
              }} />
            </>
          )}
        </div>
      ...
      {entry.needManualReview && (
        <div className="mt-3 text-xs text-amber-700 bg-amber-100 border border-amber-300 rounded px-2 py-1">
          Needs manual review{entry.manualReviewReason && `: ${entry.manualReviewReason}`}
        </div>
      )}
