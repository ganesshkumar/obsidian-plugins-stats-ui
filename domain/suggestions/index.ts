...
const canSuggestTool = (pageInfo: PageInfo, tool: ToolSuggestion, selectedTools: ToolSuggestion[]): boolean => {
  const { type, slug } = pageInfo;

  // Check if the current page is the tool page itself
  if (type === 'tool' && (tool.link.split('/')[2] ?? '') === slug)
    return false
...
const pickTools = (pageInfo: PageInfo, count: number): ToolSuggestion[] => {
  ...
  let suggestionsCount = Math.min(count, pageInfo.type === 'tool' ? tools.length - 1 : tools.length);
...
const canSuggestPost = (pageInfo: PageInfo, post: Post, selectedPosts: Post[]): boolean => {
  const { type, slug } = pageInfo;

  // Check if the current page is the post itself
  if (type === 'post' && post.id === slug)
    return false
...
const pickPosts = (pageInfo: PageInfo): Post[] => {
  ...
  const pickPostByTag = (tag: string) => {
    ...
    if (tag === 'weekly-plugin-updates') {
      const weeklyPluginPost = postsByTag[0].id === pageInfo.slug ? postsByTag[1] : postsByTag[0]
...
const pickSimilarPlugins = async (pageInfo: PageInfo) => {
  ...
  const pluginTags = currentPlugin?.osTags?.split(',')?.map(t => t.trim())?.map(t => sanitizeTag(t)) ?? [];
...
export const generateSuggestions = async (pageInfo: PageInfo): Promise<Suggestions> => {
  const { type, slug } = pageInfo
