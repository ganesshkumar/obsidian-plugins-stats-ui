...
const sanitizeTag = (tag) => {
  if (!tag || tagDenyList.includes(tag)) {
    return tag;
  }
...
module.exports = {
  siteUrl: process.env.SITE_URL || 'https://www.obsidianstats.com/',
  additionalPaths: async (config) => {
    ...
    plugins.forEach((plugin) => {
      ...
      tags.forEach((tag) => {
        ...
        if (tagsData[sanitizedTag] === undefined) {
          tagsData[sanitizedTag] = plugin.createdAt;
        } else {
          if (tagsData[sanitizedTag] < plugin.createdAt) {
            tagsData[sanitizedTag] = plugin.createdAt;
          }
        }
      });

      const category = plugin.osCategory || 'uncategorized';
      if (categoriesData[plugin.osCategory] === undefined) {
        categoriesData[category] = plugin.createdAt;
      } else {
        if (categoriesData[category] < plugin.createdAt) {
          categoriesData[category] = plugin.createdAt;
        }
      }
    });
    ...
    for (const category of categories) {
      result.push({
        ...
        lastmod: categoriesData[category],
        changefreq: 'weekly',
        priority: 0.7,
      });
    }
