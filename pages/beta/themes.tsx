...
const BetaThemes = (props: IProps) => {
  ...
  const tokens = useMemo(() => query.toLowerCase().trim().split(/\s+/).filter(t => t), [query]);
        ...
        <div className="max-w-6xl mx-auto px-2">
          ...
          <div className="my-4">
            <div className="border-l-4 border-yellow-400 bg-yellow-50 p-4 rounded">
              <div className="font-semibold text-yellow-800 mb-2">Note: Beta themes are used at your own risk</div>
              <ul className="list-disc pl-5 text-yellow-700 text-sm">
                <li>These are themes with open PRs waiting to be merged into the community themes list.</li>
          ...
          <EthicalAd type="text" style="fixed-footer" placementId="beta-themes" className='horizontal' />
          <div className="flex flex-col md:flex-row md:items-center gap-4 mb-2">
            <div className="relative md:w-1/2">
              ...
              <input ref={inputRef} value={query} onChange={handleChange} placeholder="Search beta themes" className="w-full pl-9 pr-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-violet-300" />
            </div>
          </div>
          <div className="mb-4 flex items-center gap-x-4 text-sm font-medium">
            <span className="text-gray-500">Show:</span>
            <Link href="/beta" className="underline-offset-4 hover:underline">All</Link>
            <Link href="/beta/plugins" className="underline-offset-4 hover:underline">Plugins</Link>
            <span className="underline font-semibold underline-offset-4">Themes</span>
          </div>
          {!filtered.length && <div className="text-sm text-gray-600">No beta themes match your search.</div>}
          {!!filtered.length && (
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              {filtered.map(e => <BetaEntryCard key={e.id} entry={e} highlight={query} />)}
            </div>
          )}
          {!!entries.length && <div className="mt-6 text-xs text-gray-500">* Derived from open GitHub PRs and may change.</div>}
        </div>
      </main>
      <Footer />
    </div>
...
export const getStaticProps = async () => {
  ...
  try {
    ...
    const description = 'Browse prospective beta Obsidian themes sourced from open GitHub PRs.';
    ...
    return { props: { title, description, canonical, image, entries, jsonLdSchema } };
  } catch (e) {
    console.error(e);
    return { props: { title: 'Beta Themes', description: '', canonical: '', image: '', entries: [], jsonLdSchema: null } };
