        ...
        <div className="max-w-6xl mx-auto px-2 flex flex-col h-full">
          ...
          <div className="flex justify-between items-center">
            ...
            <div className="flex gap-x-2 justify-end">
              <Button color="dark" onClick={createNewScorer} size="sm">
                New Scorer
              </Button>
...
const ScoreEditor = ({ plugins }) => {
  ...
  const setShouldValidate = useScorerFormStore(
    (state) => state.setShouldValidate
  );
  const setValidationStatus = useScorerFormStore(
    (state) => state.setValidationStatus
  );
  const isValid = useScorerFormStore((state) => state.isValid);
  const validationMessage = useScorerFormStore(
  ...
  const handleValidate = () => {
    try {
      ...
      const plugin = plugins.find(
        (plugin) => plugin.pluginId === 'better-plugins-manager'
      );
      ...
      if (!isValidResult) {
        setValidationStatus(
          false,
          'Function should update the scores inplace and should not return anything.',
          true
        );
      }
      ...
      setValidationStatus(
        false,
        'Error validating the function. Fix the function and try again.',
        true
      );
      return false;
    }
  ...
  return (
    <>
      <EthicalAd type="text" data-ea-style="fixed-footer" placementId="scorer-build-text" />
      <div className="flex justify-start flex-wrap gap-x-2 gap-y-1 mt-4">
        ...
        {metrics.map((metric, idx) => (
          <span>
            <span key={metric} className="bg-gray-100 px-1 rounded-md">
              {metric}
            </span>
      ...
      <Alert color="info" icon={Info} className="mt-4">
        <span className="font-medium">Note</span>: We have documented types the
        plugins(array of{' '}
        <a className="underline" href="/docs/global.html#PluginMetrics">
          PluginMetrics
        </a>
        ) and utils(
        <a className="underline" href="/docs/ScorerUtils.html#ScorerUtils">
          ScorerUtils
        </a>
        ) objects in{' '}
        <a className="underline" href="/docs/index.html">
          jsdocs
        </a>
        .
      </Alert>
      <Alert color="info" icon={Info} className="mt-4">
        <span className="font-medium">Note</span>: Learn to build your first
        custom score function using our{' '}
        <a
          className="underline"
          href="https://www.obsidianstats.com/posts/2025-01-18-building-a-custom-score-function"
        >
          step-by-step guide
        </a>
        .
      ...
      <div className="flex gap-x-2">
        <Button color="dark" onClick={handleValidate}>
          Validate
        </Button>
      </div>
      {validationMessage && (
...
const ScorerForm = ({ plugins }) => {
  ...
  const scoringInProgress = useScoreListStore(
    (state) => state.scoringInProgress
  );
  ...
  const handleSave = (action: 'create' | 'update', use: boolean = false) => {
    ...
    setSaveStatus(
      true,
      `Score function ${action === 'create' ? 'created' : 'updated'} successfully.`
    );
  ...
  return (
    <>
      <div className="flex flex-col gap-x-2 gap-y-2 justify-center mt-4">
        ...
        {!id && (
          <div className="flex gap-x-2">
            <Button
              color="dark"
              disabled={!isValid || !name || !description}
            ...
            <Button
              color="dark"
              disabled={!isValid || !name || !description}
              onClick={() => handleSave('create', true)}
            >
              Create and Use
            </Button>
          </div>
        ...
        {!!id && (
          <div className="flex gap-x-2">
            ...
            <Button
              color="dark"
              disabled={!isValid || !name || !description}
              onClick={() => handleSave('update', true)}
            >
              Update and Use
            </Button>
          </div>
      ...
      {saveMessage && (
        <div className={`${isSaved ? 'text-green-500' : 'text-red-500'}`}>
          {saveMessage}
        </div>
      )}
      {scoringInProgress && (
        <div className="flex gap-x-2 justify-center items-center">
          {' '}
          <Spinner aria-label="Scoring in progress" />{' '}
          <div>Scoring in progress...</div>{' '}
        </div>
      )}
      {scoringMessage && (
        <div
          className={`${scoringStatus === 'success' ? 'text-green-500' : 'text-red-500'}`}
        >
          {scoringMessage}
        </div>
      )}
...
export const getStaticProps = async () => {
  ...
  const title =
    'Build and Customize Your Own Plugin Scorer - Obsidian Plugin Stats';
  const description =
    'Discover how to build and customize your own plugin scorer for Obsidian. Learn to create custom scoring functions to evaluate plugins based on various metrics and enhance your Obsidian experience.';
