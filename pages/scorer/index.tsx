    ...
    <div className="flex flex-col min-h-screen">
      ...
      <div className="bg-white pt-5 grow">
        <div className="max-w-6xl mx-auto px-2 flex flex-col h-full">
          ...
          <EthicalAd type="text" id="scorers-text" />
                    ...
                    <Table.Cell className="w-40">
                      <div className="flex gap-x-1">
                        <Tooltip content="View Scorer">
                          <Button color="light" onClick={() => setSelectedScorer(scorer)} size="sm">
                            <Code size={12}/>
                          </Button>
                        </Tooltip>
                        <Tooltip content="Edit Scorer">
                          <Button color="light" onClick={() => openScorerBuilderPage(scorer)} size="sm">
                            <Edit size={12}/>
                          </Button>
                        </Tooltip>
                        <Tooltip content="Delete Scorer">
                          <Button color="light" className="bg-red-50 border-red-500" onClick={() => setScorerToDelete(scorer)} size="sm">
          ...
          {selectedScorer &&
            <Modal show={!!selectedScorer} onClose={() => setSelectedScorer(undefined)}>
              <Modal.Header>
                <div>{selectedScorer.name ?? ''}</div>
                <div className="text-xs text-gray-600 font-normal">{selectedScorer.name ?? ''}</div>
              </Modal.Header>
              <Modal.Body>
                <Editor
                  disabled={true}
              ...
              <Modal.Footer>
                <Button color="gray" onClick={() => setSelectedScorer(undefined)}>
                  Close
                </Button>
                <Button color="dark" onClick={() => openScorerBuilderPage(selectedScorer)}>
                  Edit
                </Button>
              </Modal.Footer>
            </Modal>
          ...
          {scorerToDelete &&
            <Modal show={!!scorerToDelete} onClose={() => setScorerToDelete(undefined)}>
              <Modal.Header> Confirm deletion </Modal.Header>
              <Modal.Body>
                Deleting "{scorerToDelete.name}" scoring function.
              </Modal.Body>
              <Modal.Footer>
                <Button color="gray" onClick={() => setScorerToDelete(undefined)}>
                  Close
                </Button>
                <Button color="failure" onClick={() => handleDeleteScorer(scorerToDelete)}>
                  Confirm
                </Button>
              </Modal.Footer>
            </Modal>
...
export const getStaticProps = async () => { 
  ...
  const title = 'Enhance your Obsidian experience with our curated list of plugins';
  const description = 'Discover how to build custom scoring functions for Obsidian plugins. Enhance your productivity by creating personalized formulas to evaluate and rank plugins based on your specific needs.';
